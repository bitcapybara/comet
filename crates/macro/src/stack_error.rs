...
use syn::{Attribute, Ident, ItemEnum, Meta, Token, Variant};
        ...
        for attr in variant.attrs {
            if attr.path().is_ident("snafu") {
                let nested = attr
                    ...
                    .expect("parse snafu error");
                for meta in nested {
                    match meta {
                        Meta::List(meta) if meta.path.is_ident("display") => {
                            let display_ts: TokenStream2 =
                                meta.parse_args().expect("parse display content error");
                ...
                //     Ok(())
                // })
                // .expect("parse snafu attributes error");
                if display.is_none() {
                    panic!("Each error should contains a display attribute");
                }
            }
